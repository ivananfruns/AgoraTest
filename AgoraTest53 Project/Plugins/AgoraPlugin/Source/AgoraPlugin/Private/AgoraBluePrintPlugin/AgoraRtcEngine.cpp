...
//FString UAgoraRtcEngine::GenerateToken(FString appId, FString appCert, FString channelName, uint32_t uid, FString uidString, uint32_t expirationTimeInSeconds)
//{
	...
	//}
	//
	//result = agora::tools::RtcTokenBuilder::buildTokenWithUid(
	//	std::string(TCHAR_TO_UTF8(*appId)), std::string(TCHAR_TO_UTF8(*appCert)), std::string(TCHAR_TO_UTF8(*channelName)), uid, agora::tools::UserRole::Role_Publisher,
	//	privilegeExpiredTs);
...
void UAgoraRtcEngine::getAudioOutputDevices(TArray<FString>& IDs, TArray<FString>& Names, FString& currentID, FString& currentName)
{

	UIAudioDeviceManager* manager = (UIAudioDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_AUDIO_DEVICE_MANAGER);

	if (manager != nullptr)
	{
		auto audioDeviceCollection = manager->EnumeratePlaybackDevices(); //RANDOMLY CRASHES due to internal access error
		if (audioDeviceCollection != nullptr)
		{
			int deviceCount = audioDeviceCollection->GetCount();

			for (int i = 0; i < deviceCount; i++)
			{
				FString deviceName;
				FString deviceID;
				audioDeviceCollection->GetDevice(i, deviceName, deviceID);
				IDs.Add(deviceID);
				Names.Add(deviceName);
				GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, deviceName);
			}
			manager->GetPlaybackDeviceInfo(currentID, currentName);
		}
		else {
			GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "audio collection not valid");
		}
	}
	else {
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "audio manager not valid");
	}
}
void UAgoraRtcEngine::getAudioInputDevices(TArray<FString>& IDs, TArray<FString>& Names, FString& currentID, FString& currentName)
{
	UIAudioDeviceManager* manager = (UIAudioDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_AUDIO_DEVICE_MANAGER);

	if (manager != nullptr)
	{
		auto audioDeviceCollection = manager->EnumerateRecordingDevices();//RANDOMLY CRASHES due to internal access error
		...
		if (audioDeviceCollection != nullptr)
		{
			...
			manager->GetRecordingDeviceInfo(currentID, currentName);
		}
		else {
			GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Audio collection not valid!");
...
void UAgoraRtcEngine::GetWebcamDevices(TArray<FString>& IDs, TArray<FString>& Names, FString& currentID)
{
	UIVideoDeviceManager* manager = (UIVideoDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_VIDEO_DEVICE_MANAGER);
	if (manager != nullptr)
	{
		auto VideoDeviceCollection = manager->EnumerateVideoDevices(); //RANDOMLY CRASHES due to internal access error
		if (VideoDeviceCollection != nullptr)
		{
			int deviceCount = VideoDeviceCollection->GetCount();
			for (int i = 0; i < deviceCount; i++)
			{
				FString deviceName;
				FString deviceID;
				VideoDeviceCollection->GetDevice(i, deviceName, deviceID);
				IDs.Add(deviceID);
				Names.Add(deviceName);
			}
			manager->GetDevice(currentID);
		}
		else
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "CANT ENUMERATE VIDEO DEVICES!");
		}
	}

}

void UAgoraRtcEngine::SetWebcamDevice(FString ID)
{
	UIVideoDeviceManager* manager = (UIVideoDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_VIDEO_DEVICE_MANAGER);
	if (manager != nullptr)
		manager->SetDevice(ID);//RANDOMLY CRASHES due to internal access error
}

void UAgoraRtcEngine::setInputAudioDevice(FString ID)
{
	UIAudioDeviceManager* manager = (UIAudioDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_AUDIO_DEVICE_MANAGER);
	if (manager != nullptr)
		auto audioDeviceCollection = manager->SetRecordingDevice(ID);//RANDOMLY CRASHES due to internal access error
}

void UAgoraRtcEngine::setOutputAudioDevice(FString ID)
{
	UIAudioDeviceManager* manager = (UIAudioDeviceManager*)QueryInterface(EINTERFACE_ID_TYPE::AGORA_IID_AUDIO_DEVICE_MANAGER);
	if (manager != nullptr)
		auto audioDeviceCollection = manager->SetPlaybackDevice(ID);//RANDOMLY CRASHES due to internal access error
}
